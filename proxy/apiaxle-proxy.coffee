...
class exports.ApiaxleProxy extends AxleApp
  ...
  authenticateWithKey: ( req, res, next ) =>
    ...
    # see if key's group matches api's group
    if req.key.data.group && req.key.data.group == req.api.data.group
      return next()
  ...
  chooseLimits: ( key, api ) =>
    ...
    if key.id.substring(0, 3) == 'ip-'
      ...
      if key.data.groupLimits
        try
          keyGroupLimits = JSON.parse key.data.groupLimits
      if keyGroupLimits && keyGroupLimits[api.id] != undefined
        qps = keyGroupLimits[api.id].qps
        qpm = keyGroupLimits[api.id].qpm
        qpd = keyGroupLimits[api.id].qpd
